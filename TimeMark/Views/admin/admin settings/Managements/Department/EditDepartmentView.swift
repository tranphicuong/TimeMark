...
struct EditDepartmentView: View {
    ...
    init(
        ...
        _name = State(initialValue: dept.department_name)
        _description = State(initialValue: dept.description ?? "")
        _selectedIcon = State(initialValue: dept.icon ?? "building.2.fill")
        _selectedColor = State(initialValue: dept.iconColor ?? "gray")
    }
    var body: some View {
        NavigationView {
            ScrollView {
        ...
        DepartmentService.shared.updateDepartment(
            ...
            DispatchQueue.main.async {
                ...
                case .success(let data):
                    print("✅ Update success:", data.name)
                    onSuccess?()
                    dismiss()
