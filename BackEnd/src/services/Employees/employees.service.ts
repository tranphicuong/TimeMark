...
//tìm tất cả các nhân sự đã xóa
export const getAllUserPlayOff = async () => {
  const snapshot = await adminDb.collection("users").get();
...
export const editUserService = async (
  ...
  const { id_position, id_department } = data;
  const positionRef = adminDb.collection("position").doc(id_position);
  const departmentRef = adminDb.collection("department").doc(id_department);
