import { FieldValue } from "firebase-admin/firestore";
import { adminDb } from "../../config/firebase.config";
import { createLeaveRequestHistory } from "./LeaveRequestHistory";

export const getLeaveRequestByStatus = async (status: string) => {
  const snapshot = await adminDb
    .collection("leave_request")
    .where("status", "==", status)
    .orderBy("created_at", "desc")
    .get();

  const now = new Date();

  const requests = await Promise.all(
    snapshot.docs.map(async (doc) => {
      const data = doc.data();

      // 🔥 chỉ auto cancel khi đang lấy pending
      if (status === "pending") {
        const leaveTime = data.from_date?.toDate();

        if (leaveTime) {
          const diffHours =
            (leaveTime.getTime() - now.getTime()) / (1000 * 60 * 60);

          if (diffHours <= 2) {
            await doc.ref.update({
              status: "cancelled",
              cancelled_at: new Date(),
            });
            await createLeaveRequestHistory({
              leave_request_id: doc.id,
              action: "cancelled",
              approved_by: data.approved_by,
              note: null,
            });
            return null;
          }
        }
      }

      // 👉 user info
      let userData = null;
      if (data.id_user) {
        const userSnap = await data.id_user.get();
        if (userSnap.exists) {
          const u = userSnap.data();
          userData = {
            id: userSnap.id,
            email: u?.email || null,
            name: u?.name || null,
          };
        }
      }

      // 👉 approved by
      let approved_by = null;
      if (data.approved_by) {
        const approvedSnap = await data.approved_by.get();
        if (approvedSnap.exists) {
          const u = approvedSnap.data();
          approved_by = {
            id: approvedSnap.id,
            email: u?.email || null,
            name: u?.name || null,
          };
        }
      }

      // 👉 leave type
      let leaveTypeData = null;
      if (data.id_leave_type) {
        const leaveSnap = await data.id_leave_type.get();
        if (leaveSnap.exists) {
          const l = leaveSnap.data();
          leaveTypeData = {
            id: leaveSnap.id,
            name: l?.name || null,
          };
        }
      }

      return {
        id: doc.id,
        user: userData,
        leave_type: leaveTypeData,
        from_date: data.from_date,
        to_date: data.to_date,
        reason: data.reason,
        status: data.status,
        approved_by,
        approved_at: data.approved_at,
        created_at: data.created_at,
      };
    }),
  );

  return requests.filter(Boolean);
};
//
export const approveLeave = async (
  id: string,
  data: {
    status: string;
    approved_by: string;
    note: string;
  },
) => {
  const approved_by_ref = adminDb.doc(`users/${data.approved_by}`);
  //tạo lịch sử khi approved
  await createLeaveRequestHistory({
    leave_request_id: id,
    action: data.status,
    approved_by: data.approved_by,
    note: data.note,
  });
  //cập nhật trên firebase
  await adminDb.collection("leave_request").doc(id).update({
    status: data.status,
    approved_by: approved_by_ref,
    approved_at: FieldValue.serverTimestamp(),
  });

  return {
    id,
    status: data.status,
  };
};
